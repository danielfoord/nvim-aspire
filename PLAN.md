...
**Known limitations** (document in README/help): container-backed resources aren't attachable; `:AspireStop`'s descendant kill still shells out to `pgrep`, a no-op on Windows, but this doesn't matter functionally — DCP's own monitor-based shutdown handles child cleanup on every platform (see Windows support note above); `:AspireStop` while a DAP session is attached leaves that session pointing at a dead process — nvim-dap handles a disconnected adapter reasonably but this isn't actively cleaned up in v1.
